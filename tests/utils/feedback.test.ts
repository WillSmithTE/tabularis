...
describe("feedback", () => {
  describe("buildFeedbackPayload", () => {
    it("should contain exactly the documented fields and nothing else", () => {
      // Privacy guarantee: the payload is the user's message, the category
      // they picked, and a static app identifier + version. If this test
      // fails because a field was added, that addition must be justified in
      // review as a privacy-relevant change.
      const payload = buildFeedbackPayload("hello", "bug");
      expect(Object.keys(payload).sort()).toEqual(["clientId", "comment", "metadata"]);
      expect(Object.keys(payload.metadata).sort()).toEqual(["app", "appVersion", "category"]);
    });

    it("should carry the user message trimmed", () => {
      const payload = buildFeedbackPayload("  the filter panel resets  ", "bug");
      expect(payload.comment).toBe("the filter panel resets");
    });

    it("should use the static app identifier and current version", () => {
      const payload = buildFeedbackPayload("x", "idea");
    ...
    it("should pass through each supported category", () => {
      for (const category of FEEDBACK_CATEGORIES) {
        expect(buildFeedbackPayload("x", category).metadata.category).toBe(category);
      }
    });

    it("should never include page or window context", () => {
      const serialized = JSON.stringify(buildFeedbackPayload("x", "other"));
